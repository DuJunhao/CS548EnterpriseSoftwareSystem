...
public class InitBean {
	...
	private void init(){
		...
		Treatment drugTreatment=treatmentFactory.createDrugTreatment("canc33er3", "panacea", (float) 15.0);
		Treatment surgery=treatmentFactory.createSurgeryTreatment("asthema12", dob);
		Patient john=patientFactory.createPatient(488856789L, "helloworld123", dob, 22);
...
*/
		...
		Provider paul=providerFactory.createProvider("oncology",512399412L);
		Provider alex=providerFactory.createProvider("surgery",7657361L);
		...
		surgery.setPatient(john);
		surgery.setProvider(alex);
		
		john.addTreatment(drugTreatment);
		logger.info("Added Treatment "+drugTreatment.getTreatmentType()+" "+drugTreatment.getId()+" for patient "+john.getName()+" id="+john.getId()+" Pid="+john.getPatientId());
		john.addTreatment(surgery);
		logger.info("Added Treatment "+surgery.getTreatmentType()+" "+surgery.getId()+" for patient "+john.getName()+" id="+john.getId()+" Pid="+john.getPatientId());
